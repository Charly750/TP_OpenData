...
import type { NutrientInfo } from "@/app/types";
...
export default function Produits() {
		...
		const fetchSearchResults = async () => {
			...
			try {
				...
				const response = await fetch(
					`http://localhost:5000/search/${searchType}/${encodeURIComponent(
						searchTerm
					)}`,
					{
						headers: {
	...
	const fetchProductByBarcode = async (code: string) => {
		try {
			...
			const response = await fetch(
				`http://localhost:5000/product/${code}`,
				{
					headers: {
						Authorization: `Bearer ${token}`,
					},
	...
	const getNutriscoreColor = (score: string) => {
		...
		return (
			colors[score.toLowerCase() as keyof typeof colors] || "bg-gray-400"
		);
	};

	//Fonction pour passer les details d'un produits via le localStorage
	const goToDetails = (productId: string) => {
		localStorage.setItem("selectedProduct", JSON.stringify(products));
		localStorage.setItem("selectedNutrients", JSON.stringify(nutrients));
		router.push("/detail");
	};
				...
				<header className="flex flex-col md:flex-row justify-between items-center mb-8 gap-4">
					<div>
						...
						<p className="text-emerald-600 mt-1">
							Découvrez des produits sains et écologiques
						</p>
					</div>
								...
								>
									<div className="relative h-56 overflow-hidden">
										...
										<div className="absolute top-3 right-3">
											<span
												className={`inline-flex items-center justify-center w-8 h-8 rounded-full text-white font-bold ${getNutriscoreColor(
